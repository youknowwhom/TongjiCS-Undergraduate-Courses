    ...
    def get_goto_table(self):
        ...
        for i in range(len(self.closures)):
            ...
            for j in range(len(self.non_terminal_symbols)):
                if j in self.gos[i].keys():
                    goto_table[i + 1][j + 1] = str(self.gos[i][j])
        return goto_table
    ...
    def getParse(self, lex):
        ...
        while index < len(lex):
            ...
            if current_state[token_id][0] == ACTION_S:
                ...
                next_state_id = current_state[production.from_id]
                item = {
                    "state": next_state_id,
                    "tree": {
            ...
            elif current_state[token_id][0] == ACTION_ACC:
                return stack[-1]["tree"]
